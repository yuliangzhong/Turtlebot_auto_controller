...
class Robot:
    def __init__(self, cfg: Config):
        ...
        self.pos_ramp = VelocityRamp(cfg.velocity_max / cfg.voxel_size, cfg.acceleration / cfg.voxel_size)
    ...
    def initialize_in_world(self, world):
        ...
        while True:
            self.position_x = random.randint(0, np.shape(self.world.map_gt)[0] - 1)
            self.position_y = random.randint(0, np.shape(self.world.map_gt)[1] - 1)
            if self.cfg.initial_safety_margin <= 0:
                if self.world.map_gt[self.position_x, self.position_y] == 0:
                    break
            else:
                offset = int(self.cfg.initial_safety_margin / self.cfg.voxel_size)
    ...
    def bound_query_voxels(self, x, y):
        ...
        x_out = np.maximum(np.minimum(np.round(x), np.shape(self.observed_map)[0] - 1), 0)
        y_out = np.maximum(np.minimum(np.round(y), np.shape(self.observed_map)[1] - 1), 0)
    ...
    def take_sensor_measurement(self):
        ...
        n_rays = self.cfg.camera_range * math.pi * 2 * self.cfg.camera_fov / 360 / self.cfg.voxel_size
        ...
        for i in range(n_rays):
            ...
            while r <= self.cfg.camera_range / self.cfg.voxel_size:
                r = r + 1
                x, y = self.bound_query_voxels(self.position_x + r * math.cos(phi), self.position_y + r * math.sin(phi))
                self.observed_map[x, y] = self.world.map_gt[x, y]
                if self.observed_map[x, y] == 1:
                    break

    def check_move_feasible(self, x, y, step=None, vect=True):
        """ Check whether move collides and is safe during planning """
        if step is None: step = self.cfg.voxel_size / 2  # double resolution to catch all voxels
        n_steps = math.ceil((x ** 2 + y ** 2) ** 0.5 / step)
        x_vals = self.position_x + np.linspace(0, x, max(1, n_steps))
        y_vals = self.position_y + np.linspace(0, y, max(1, n_steps))
        if vect:
            px, py = self.bound_query_voxels(x_vals, y_vals)
            #print("px, py = ",px, py)
            obs_map = self.observed_map[px, py] != 0
            coll_map = obs_map #self.world.map_gt[px, py] != 0
            #print(obs_map)
            is_safe = sum(obs_map) == 0
            coll_step = np.argmax(coll_map)
            try:
                has_collided_at = float(coll_step) / n_steps if (coll_step > 0) or \
                                                                (coll_step == 0 and coll_map[0]) else None
            except:
    ...
    def move_to(self, x, y, yaw):
        ...
        self.yaw_ramp.setup_from_goal(math.fabs(yaw_diff))  # requires positive sign, so compensate for it later
        ...
        self.x_buff[2], self.y_buff[2], self.yaw_buff[2] = x + self.x_buff[0], y + self.y_buff[0], yaw
        for i in range(math.ceil(max_time * self.cfg.camera_rate) + 1):
            ...
            self.yaw = self.yaw_buff[0] + np.sign(yaw_diff) * self.yaw_ramp.x(t)
    ...
    def save_images(self, action=None, collision=False):
        ...
        if not self.init_display:
            ...
            self.image = plt.imshow(robot_map, cmap=robot_cmp, interpolation='none')
            self.line1, = self.plot_ax.plot(0., 0., linestyle=':', color='orange')
            self.plot_ax.fill_between([.0, .1], [.0, .1], color='k', alpha=0.25)
            plt.axis([0, np.shape(self.world.map_gt)[0] - 1, 0, np.shape(self.world.map_gt)[1] - 1])
            plt.axis('off')
            self.fig.savefig(self.cfg.images_folder + str(self.file_name).zfill(8) + '.png', bbox_inches='tight', pad_inches=0)
            self.file_name += 1
        ...
        x_lim = np.clip(x_new + self.cfg.local_submap_size_x/2 * np.array([[-1, -1], [+1, +1]]), 0., max_lim)
        y_lim = np.clip(y_new + self.cfg.local_submap_size_y/2 * np.array([-1, +1]), 0., max_lim)
        robot_map = self.observed_map
        ...
        self.plot_ax.fill_between(y_lim, x_lim[0], x_lim[1], facecolor='k', alpha=0.1, edgecolor='k')
    ...
    def get_local_submap(self):
        x_start = math.floor(self.position_x - self.cfg.local_submap_size_x / 2)
        y_start = math.floor(self.position_y - self.cfg.local_submap_size_y / 2)
        ...
        if y_end > y_max:
            ...
            y_end = y_max
        local_submap = np.ones((int(self.cfg.local_submap_size_x), int(self.cfg.local_submap_size_y))) * 2
